...
## 11. N-Queen Problem (Backtracking)
- The N-Queen problem is to place N chess queens on an N×N chessboard so that no two queens attack each other.

### **Technique Used**
- Backtracking: Try placing queens one by one in different columns and check for safety.

### **Time Complexity**
$$
O(N!)
$$ 
because we try all possible configurations.
